...
def _organize_train_dir():
    ...
    _save_images_to_dir(NEW_TRAIN_DIR, paths[ind_tr], labels[ind_tr], classes, multi_crop=True)
    _save_images_to_dir(
...
def _train_val_split_indices(labels):
    # just 1 fold (so not really cross validation, but simply splits data into train and val)
